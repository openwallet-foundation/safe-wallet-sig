...
| ------ | -------------- | -------------------------- | --------------------- |
| v1.0   | 10th July 2024 | Final editorial pass, add comments from Safe Wallet SIG | Juliana Cafik         |
| v0.963   | 27th June 2024 | Full Edit for ‘tone’ consistency, reduce duplications, add additional detail and/or context, to align under 4 Pillars & add Executive Summary     | Juliana Cafik         |
| v0.962   | 18th June 2024 | Finalised.                 | Andy Tobin            |
...
- a. Unique identifiers specific to the holder or wallet, which may be shared without the holder’s
consent during each transaction, should be avoided. These can be considered as follows:
  - i. Akin to a tracking beacon that allows colluding entities to correlate all activities of a
holder using that wallet
  - ii. Dangerous when proxy verification services that process verifications for many relying
parties can see and track identifiers and profile the behaviors of all holders
  - iii. A wallet serial number in the “metadata” of the credential exchange transaction
  - iv. The exposure of IP addresses used in digital interchanges should be factored into the
overall operational risk profile
- b. Credential-specific unique identifiers that are shared, without user choice, are another class of
unique identifiers that should be avoided and include:
  - i. Identifiers specific to a credential, which a holder inevitably shares, can lead to
...
2. Decoupling issuers and verifiers are a fundamental aspect of implementing privacy and aligning with
...
- a. Reduced Data Exposure: If issuers and verifiers are distinct, the personal data revealed during
transactions can be kept to a minimum. The party verifying the credential doesn’t need to know
its issuer, and vice versa
- b. Avoiding Correlation: Keeping issuers and verifiers separate helps avoid the correlation of
...
- c. Increased User Autonomy: Users have more control over their data when issuers and verifiers are
...
- d. Trust Building: Trust among users is fostered when issuers and verifiers are decoupled, as users
...
- e. Issuers and verifiers of digital credentials should not be able to track their use which would
...
- f. Issuers should not be able to aggregate holder information
- g. Preventing verifiers from contacting issuers (a process known as decoupling) for any reason,
...
avoided
- h. Issuers and Verifiers should not be able to access a digital wallet without explicit holder Notice &
Consent
3. Support for Privacy-Enhancing Credential Sharing Mechanisms in Wallets. The wallet should
...
4. Privacy of Wallet Transactions should be preserved and the entities that provide the code and/or
...
- a. Code providers & contributors and/or cloud operators of digital wallets must not be able to
observe or track transactions undertaken by holders
- b. This is particularly important in the case of hybrid and cloud wallets, where transactions are
performed on the wallet operator’s infrastructure
- c. Wallet code providers and cloud operators must not be able to access the contents of digital
wallet transactions log files or backups
- d. Wallet code providers and cloud operators must not have a “master key” or “back-door” that
enables them to access the contents of wallets

5. Digital Wallet ecosystem providers or organizations that provide the infrastructure to which digital
...
are:
- a. In situations, such credential exchange platforms will need to “translate” from an encrypted
...
holder privacy.
- b. Proxy services may provide credential issuance/verification services for many issuers and
verifiers. In doing so, they could observe and correlate wallet transactions across large
ecosystems and should:
  - i. Not observe, store and correlate wallet transactions
  - ii. Provide clear Notice and Consent that they are dealing with a proxy service rather than
the true end issuer/verifier
6. Wallet Consent management is a key component of privacy by design, ensuring that privacy is not an
...
- a. User Control: Consent management gives users control over their own data. Users decide what
information they want to share, with whom, and for what purpose. This empowers users and
respects their privacy
- b. Minimizing Data Exposure: With consent management, users can choose to share only the
...
- c. Trust and Transparency: Consent management promotes trust as users know their data is being
used responsibly. It also ensures transparency as users are aware of how their data is being used
- d. Regulatory Compliance: Many regions have data protection regulations that require user consent
...
- e. Preventing Misuse of Data: By giving users the ability to manage consent, it reduces the risk of
data misuse. Holders can withdraw consent if they believe their data is not being used
appropriately
f. Provide clear, understandable, and easy-to-use mechanisms for the user to provide consent to
...
g. Record of Notice & Consent: Holders must be able to track the Notices provided and consents
given and should be able to withdraw that consent at any time
h. Enable Wallet privacy preserving supporting functions: The act of providing consent by the holder
...
unauthorized access, phishing attacks, man-in-the-middle, replay & relay attacks, malware & elevated
...
The Digital Identity & Authentication Council of Canada’s (DIACC's) Digital Wallet Component
...
Security Design Controls & Considerations that may be necessary to provide safety for wallet holders as
...
- a. Malware:
  - i. Regular Software Updates: Continually update wallet software, operating systems,
internet browsers, phone firmware, and antivirus software to fix known vulnerabilities as
they are discovered
  - ii. Secure connections and end-point management for all network interactions, including
NFC and Bluetooth low-energy enabled hardware
  - iii. Phishing resistant authentication
  - iv. Obtain wallet software from official sources
  - v. Root detection for mobile devices
- b. Social Engineering prevention involves a combination of technical measures, policy enforcement
and wallet holder education. Strategies for mitigation include:
  - i. User Education: Users should be educated about the common tactics used in social
engineering attacks and how to identify potential scams
  - ii. Phishing resistant authentication
  - iii. End-point security
  - iv. Regular audits of accounts and access levels
  - v. Software updates to prevent evolving known vulnerabilities
  - vi. Policy enforcement for policies and procedures that consider potential social engineering
vectors and provide specific guidance on steps to take to avoid successful attacks
  - vii. Brute Force Attack Mitigation options:
    - Phishing resistant authentication
    - Account lockouts and rate limiting
    - Strong data encryption & storage
  - viii. End-point authentication to prevent Man-in-the-Middle Attacks such as:
    - Relay Attacks: Relaying information between parties
    - Replay Attacks: Capturing and reusing information
2. Key Management & Generation: the security of a cryptographic system heavily relies on the secure
...
- a. Random Number Generator Quality: It is important to use a high-quality random number
generator for key generation. This helps to avoid predictability and strengthens the security of
the cryptographic system
- b. Encryption Algorithms and Key Size Selection: The choice of encryption algorithms and key size
should be based on the security requirements of the application and the most secure
combination should be selected
- c. Safe Key Storage: Keys should be stored securely using hardware security modules, smart cards,
and/or trusted platform modules. This secure storage helps to prevent unauthorized access to
the keys
- d. Key Rotation and Decommissioning: Keys should be renewed periodically, and old keys should be
decommissioned. Regular renewal of keys reduces the risk of key compromise over time
- e. Preventing Key Compromise: The secure management of cryptographic keys is of utmost
...
the keys
- f. Adherence to Standards: Established standards and recommendations for key management
...
- g. Backup and Recovery: A robust key recovery system, or process, is critical to ensure users can
regain access to their wallet(s)if the private keys are lost or compromised
- h. Usage: Keys should be protected while in use, and at rest, in a secure manner & within a secure
environment
- i. Key Revocation: Processes for revoking and replacing compromised keys
- j. Audit: Processes for logging, detecting and providing notice of security risks to keys
- k. Private Key Management to prevent exposure during the rendering of transaction processes
3. Credential Signature / Format: A credential signature in the context of a digital wallet is a digital proof
...
- a. Credential Signature Verification: mechanisms to verify the authenticity of the credential
signatures to ensure they have not been tampered with and are issued by a legitimate trusted
authority
- b. Secure Storage of Credentials: Secure storage to prevent unauthorized access and tampering
with verified signatures
- c. Privacy Preservation: Mechanisms to support the verification of credentials without revealing
unnecessary personal information, such as selective disclosure and zero-knowledge proofs
4. Credential Management: Credential management in the context of a digital wallet refers to the
process of handling and safeguarding digital credentials to ensure their integrity. Security
considerations are:
- a. Storage: Credentials must be stored securely to prevent unauthorized access.
- b. Backup and Recovery: A robust recovery system, or process, to ensure holders are able to regain
access to their wallet(s)
- c. Usage: Digital wallet architecture must support credential use in a secure manner & within a
secure environment
- d. Revocation: The process of revoking and replacing compromised credentials is a crucial aspect of
...
vary by issuer, wallet, and framework and will need to ensure:
  - i. Adherence to privacy regulations and principles
  - ii. Revocation checks cannot be linked by issuers and third parties
  - iii. Prevent holder traceability
- e. Audit: Auditing processes are crucial for maintaining the security of a digital wallet. They involve
...
5. Presentation Protocols: Presentation protocols in the context of a digital wallet refer to the
...
- a. Phishing-resistant user authentication to ensure the holder presenting the credentials is the
legitimate holder of both the wallet and the credential
- b. Secure, Phishing and ‘spoof’ resistant user interface
- c. Strong data encryption and storage to secure credentials and prevent unauthorized access
- d. Digital wallet rendering and user interface to ensure that holders are interacting with intended
functions to support credential presentation and usage in a secure manner
- e. Privacy-preserving mechanisms to support the presentation of credentials without revealing
unnecessary personal information, such as selective disclosure and/or zero-knowledge proofs
- f. Audit processes for logging and detecting security risks, such as malware, phishing attacks, man-
in-the-middle attacks, etc., that may impact credential presentation.
6. Mechanisms to convey consent: User Consent is a critical aspect of privacy and security in digital
...
- a. Phishing-resistant user authentication to ensure the holder providing consent is the legitimate
holder of both the wallet and the credential
- b. Phishing and ‘spoof’ resistant user interface
- c. Strong data encryption and storage to secure sensitive data and consent records and prevent
unauthorized access
- d. Revocation mechanisms to provide holders with the option to revoke consent such as a protocol
...
2. Open Design: A concept that involves the development of products, machines, and systems using
...
- a. Security should be integrated from the start of the design process. This approach, known as
Security by Design, aims to incorporate security considerations into the very foundations of a
system
- b. The principle of open design states that security systems should not rely on secret designs or
...
- c. Security should be viewed holistically and, wherever possible, multiple levels of security
...
- d. Limiting the amount of code and functionality exposed to potential attackers. The less code,
the fewer vulnerabilities there are likely to be
3. Attack Surface Minimization: A security strategy that aims to reduce the number of potential
...
4. Defense in depth: A security strategy that employs multiple layers of protection to slow down or
...
5. Least privilege: A security concept that requires each user, system, or process related to the wallet to
...
is compromised
6. Need to know: A security concept that states that access to information should be restricted to
...
7. Role Based Access Controls (RBAC): A method of regulating access to resources based on the roles of
...
8. Separation of Duties: A security principle that shifts the burden of security as much as possible away
...
9. Secure by Default: A principle aimed at shifting the burden of security as much as possible away from
...
10. Security Controls: such as NIST 800-53 and PCI DSS are safeguards, or countermeasures, prescribed to
...
11. Software Bill of Materials (SBOM): An SBOM is a key building block in software security and software
...
- a. Handling Vulnerabilities: SBOMs allow organizations to keep track of third-party components
...
- b. Enhanced Visibility and Transparency: SBOMs improve visibility in the software supply chain. They
...
deployment
- c. Incident Response and Forensics: In case of a security incident, an SBOM can assist cybersecurity
...
- d. Regulatory Compliance: Many governments and regulatory organizations now require the use of
...
licensing terms
- e. Efficiency: The common infrastructure and data exchange format of SBOMs could save
companies time by fostering greater collaboration between organizations
12. Software Development Best Practices, Software Assurance Models and Standards: should be
...
wallets such as:
- a. Quality Assurance: Compliance with best practices and standards ensures the software’s
quality, leading to a more reliable and robust digital wallet
- b. Security: Best practices often encompass security measures such as encryption, secure APIs,
and regular updates, which help safeguard user data and transactions
- c. Scalability: Adhering to best practices allows for scalability, ensuring that the digital wallet
can accommodate growth in the user base and transaction volume
- d. Interoperability: Standards ensure interoperability, enabling the digital wallet to interact
seamlessly with various systems and technologies
- e. Regulatory Compliance: Compliance with standards can assist in meeting regulatory
requirements, reducing legal risks
- f. Trust: Following recognized standards, and best practices can enhance trust among users and
partners
- g. Risk Management: Assurance models aid in identifying and managing risks, contributing to
the overall security and reliability of the digital wallet
- h. Validation: They offer a framework for validating that the digital wallet meets specified
requirements and standards
- i. Continuous Improvement: Assurance models support continuous improvement by providing
benchmarks and metrics for evaluating performance
- j. Stakeholder Confidence: Demonstrating that the digital wallet has been thoroughly evaluated
and meets recognized standards, assurance models can increase confidence among
stakeholders

## Pillar 3: Supporting Functions

1. Wallet & Device Locking and Unlocking Mechanisms: A safe digital wallet will provide a secure and
...
specific to wallet lock and unlock:
    - a. The wallet locks if focus is lost
    - b. The wallet locks after a pre-defined period of inactivity
    - c. On mobile devices, application thumbnails (displayed during app switching or selection) are blank
or otherwise rendered unreadable to protect sensitive information
    - d. If implemented, the wallet must properly secure the PIN/Password and must not persist the
PIN/Password in memory after an authentication event

2. Authentication: Authentication and device locking/unlocking in the context of digital wallets are
...
highlight the following for context:
    ...
    - a. The mechanisms and controls to lock and unlock and authenticate to a wallet are defined by
...
implementation and include, but are not limited to:
      - i. PIN or password requirements: A PIN or password should implement complex rules to
...
specifically detailed
      - ii. Multi-Factor Authentication (MFA), as defined by NIST SP 800-63-4 B (draft), is an
...
with an integrated biometric sensor that is required to activate the device
      - iii. Phishing-resistant authentication is a method to prevent attackers from gaining access to
...
Examples include:
        ...
        - FIDO Passkeys
      - iv. Step-up authentication is a security measure that is distinct from multi-factor authentication
...
3. Holder Binding: Digital wallets are applications used to receive, store, and share attributes in a secure,
...
credential).
    ...
    - a. Holder binding to the device and wallet:
       - i. The wallet should have a mechanism to associate, and differentiate, one or more natural
persons to the device as part of the wallet provisioning & setup process
       - ii. Where biometric binding is used, the wallet should have a mechanism to store, and share,
associated metadata such as: signature of authority providing the biometric data, live capture
by authority indicator, quality assessment method & result, presentation attack detection
(PAD) method & result, morph attack detection (MAD) method & result
       - iii. The wallet should have a mechanism to store, and share, the associated assurance level of
the issuance process (e.g., reflective of in-person, remote supervised, remote; uniqueness
determination )
       - iv. The wallet should take any action if a previously set up device security (e.g. a fingerprint) is
changed (e.g. the addition of a new fingerprint which could come from another person)
       - v. The wallet may have its own person binding method in addition to the one implemented by
the device’s operating system
    - b. Holder binding to their credentials at issuance:
       - i. The wallet should provide a mechanism whereby the issuer is able to confirm that the person
they are issuing the credential to is the correct person.
       - ii. The issuer may be able to insert into the credential (as an attribute or metadata)
...
relying party can confirm what checks the issuer carried out
    - c. Holder binding to their keys (see Key Management in the Security section):
       - i. The wallet must have a mechanism to bind the private keys within the wallet that are used
...
place in each of these bindings
       - ii. This mechanism should operate within a secure enclave or trusted execution environment.
    - d. Holder binding at the point of presentation:
       - i. The wallet should have a mechanism to check that the person releasing a presentation
(“proof”) to a relying party is the same person to whom the credential was issued
       - ii. The wallet may be able to inform the relying party what mechanism it used to perform such a
check and provide evidence of the success of the check
       - iii. The wallet should support multiple methods for executing holder binding at the point of
presentation e.g. PIN entry, biometrics
       - iv. The wallet should have the ability to configure the holder binding checking it does in
...
4. Additional information regarding biometrics in holder binding: In each of the Holder binding sub-
...
summarized below:
    - a. Legislation that organizations need to adhere to while processing data. E.g., Article 30 of the
...
use personal data
    - b. Legislation that ensures the digital rights of the individuals are considered. The GDPR has a
chapter on the rights of data subjects (individuals) which includes the right of access, the right to
rectification, the right to erasure, the right to restrict processing, the right to data portability, the
right to object and the right not to be subject to a decision based solely on automated
...
2. Certification: Certification in the context of digital wallets relates to the formal evaluation of products
...
relating to the certification compliance of their wallet for its intended usage scenarios including:
    - a. Compliance with regulations: The wallet complies with national or regional certification
requirements. For example the eIDAS Revision regulation, where EUDI Wallet certification is
mandatory.
    - b. Components to be certified and/or attested to:
      - i. Does the digital wallet mobile app have to be certified, and to which certification standard? If
so, is the digital wallet certified?
      - ii. Does the secure element in the mobile device have to be certified, and to which certification
standard? If so, is the secure element certified? For example: Common Criteria or
GlobalPlatform certification
      - iii. Does the digital wallet backend have to be certified, and to which certification standard? For
example: ISO 27001 audit
      - iv. Are there any other components in the digital wallet eco-system to be certified, and to which
...
Common Criteria or FIPS 140-3 certified
    - c. Evidence of certification:
      - i. The digital wallet provider must supply evidence to the holder that its certification is up to
date
      - ii. Is there a certification body (in the applicable country/region) that publishes lists of certified
digital wallets?
      - iii. Does the certification body publish certificates or reports about the digital wallet
certifications?

3. Counterparties: In the context of a digital wallet, a counterparty is a person, organisation or thing to
...
credentials, an online shop requesting age verification, a police officer, or a passport e-gate etc.
    - a. Counterparty Identity Verification
      - i. The wallet should inform a holder that the counterparty that they are interacting with is who
...
organization (or another user) to determine that they are legitimate
      - ii. The holder should be able to see several levels of detail about the counterparty. For example:
...
obligations the counterparty may have regarding revealing the shared information
     - iii. The holder should be able to quickly and easily determine that the counterparty is allowed to
...
4. Audit: Digital wallets have a strong connection with Privacy and Legislative Compliance issues. There
...
include:
    - a. User Audit: Holders of a digital wallet should be able to view all their transactions, use them as
evidence of activities such as proof of purchase, and can erase their transaction history
    - b. Legal Audit:
i. The digital wallet may need to provide an activity log for one or more actions under legal
authority
ii. Digital wallets, like any other technology, are subject to local laws and regulations. In certain
...
5. Accountability: Accountability: Digital wallets facilitate access to services from various entities, and
...
Digital Wallets, the prerequisites for accountability can be bifurcated into:
    - a. Technical strategies, which involve the use of hardware, software, and protocols
